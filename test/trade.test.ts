...
describe('Trade', () => {
  ...
  const pair_0_1 = new Pair(new TokenAmount(token0, BigInt(1000)), new TokenAmount(token1, BigInt(1000)))
  const pair_0_2 = new Pair(new TokenAmount(token0, BigInt(1000)), new TokenAmount(token2, BigInt(1100)))
  const pair_0_3 = new Pair(new TokenAmount(token0, BigInt(1000)), new TokenAmount(token3, BigInt(900)))
  const pair_1_2 = new Pair(new TokenAmount(token1, BigInt(1200)), new TokenAmount(token2, BigInt(1000)))
  const pair_1_3 = new Pair(new TokenAmount(token1, BigInt(1200)), new TokenAmount(token3, BigInt(1300)))
  ...
  const empty_pair_0_1 = new Pair(new TokenAmount(token0, BigInt(0)), new TokenAmount(token1, BigInt(0)))

  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(new Route([pair_weth_0], ETHER), CurrencyAmount.ether(BigInt(100)), TradeType.EXACT_INPUT)
  ...
  describe('#bestTradeExactIn', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactIn([], new TokenAmount(token0, BigInt(100)), token2)).toThrow('PAIRS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        Trade.bestTradeExactIn([pair_0_2], new TokenAmount(token0, BigInt(100)), token2, { maxHops: 0 })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(Trade.bestTradeExactIn([empty_pair_0_1], new TokenAmount(token0, BigInt(100)), token1)).toHaveLength(0)
    })
    ...
    it('insufficient input for one pair', () => {
      const result = Trade.bestTradeExactIn([pair_0_1, pair_0_2, pair_1_2], new TokenAmount(token0, BigInt(1)), token2)
      expect(result).toHaveLength(1)
      expect(result[0].route.pairs).toHaveLength(1) // 0 -> 2 at 10:11
      expect(result[0].route.path).toEqual([token0, token2])
    ...
    it('no path', () => {
      const result = Trade.bestTradeExactIn([pair_0_1, pair_0_3, pair_1_3], new TokenAmount(token0, BigInt(10)), token2)
  ...
  describe('#bestTradeExactOut', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactOut([], token0, new TokenAmount(token2, BigInt(100)))).toThrow('PAIRS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        Trade.bestTradeExactOut([pair_0_2], token0, new TokenAmount(token2, BigInt(100)), { maxHops: 0 })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(Trade.bestTradeExactOut([empty_pair_0_1], token1, new TokenAmount(token1, BigInt(100)))).toHaveLength(0)
    })
